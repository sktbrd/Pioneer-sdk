...
export function DappsExplore({usePioneer}: any) {
  ...
  const handleHomepageChange = (event:any) => {
  ...
  const handleSelectedBlockchains = (selectedOptions:any) => {
    const selectedBlockchains = selectedOptions.map((option:any) => option.value);
    setBlockchainsSupported(selectedBlockchains);
  };

  const handleSelectedProtocols = (selectedOptions:any) => {
  ...
  const handleSelectedFeatures = (selectedOptions:any) => {
    const selectedFeatureValues = selectedOptions.map((option:any) => option.value);
  ...
  const columns = React.useMemo(() => {
    const columnDefinitions = [
      ...
      columnHelper.accessor("score", {
        ...
        // @ts-ignore
        footer: (info) => Math.round(Number(info.column.id)),
      }),
      columnHelper.accessor("name", {
        id: "upvote",
        cell: (info) => (
  ...
  const openEntry = async (name: string) => {
    try {
      ...
      // @ts-ignore
      const selectedEntry = data.find((entry:any) => entry.name === name);
      if (!selectedEntry) {
  ...
  const DownVotesTable = () => (
    <>
      ...
      <Table variant="striped" colorScheme="teal">
        ...
        <Tbody>
          {allDownVotesContext.map((vote:any, index:any) => (
  ...
  const handleShowUnchartedChange = (event:any) => {
    //update data
  ...
  const handleBlockchainFilterChange = (selectedOption:any) => {
    setSelectedBlockchain(selectedOption);
  };

  // Function to handle the Wallet filter select change
  const handleWalletFilterChange = (selectedOption:any) => {
